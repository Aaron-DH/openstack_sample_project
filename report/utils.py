...
from report.client import cache
from report.client import keystone_client
from report.openstack.common._i18n import _, _LE, _LW
        ...
        for key in module_data.keys():
            ...
            if isinstance(module_data[key], pyclbr.Class):
                ...
                for method, func in inspect.getmembers(clz, inspect.ismethod):
                    setattr(clz, method, decorator
                            ("%s.%s.%s" % (module, key, method), func))
...
def tztime2time(tztime_str):
    time_str = datetime.datetime.strptime(tztime_str, "%Y-%m-%dT%H:%M:%SZ")
    return time_str

'''
def token_get(uri, url, token_opts=None, cache_opts=None):
    """
    :param uri: ksystone service's host and port.
    for example:uri = http://127.0.0.1:5000
    :param url: url of RESTful API
    for example:url = '/v2.0/tokens'
    :param token_opts: options for get token
    token_opts = {
        "username": "username",
        "password": "password",
        "project_name": "project_name"
    }
    :param cache_opts:options for memcache
    cache_opts = {
        "host": "127.0.0.1",
        "port": 11211
    }
    :return:
    {
        'issued_at: '2015-09-28T03:37:54.531438',
        'expires: '2015-09-28T04:37:54Z',
        'id': '161a44a48cc44e6bade5b170245937af',
        'audit_ids: ['7OIEnwVEThScwV57oAKY7w']
        }
    """
    mem_cli = TokenCache(cache_opts)
    if not mem_cli.get_token('token'):
        ks_cli = TokenClient(uri=uri)
        passwordCredentials = dict(username=token_opts.get('username'),
                                   password=token_opts.get('password'))
        auth = dict(tenantName=token_opts.get('project_name'),
                    passwordCredentials=passwordCredentials)
        data = dict(auth=auth)
        token = ks_cli.get_token(url, data)
        if token:
            TZexpires = token.get('expires')
            expires = tztime2time(TZexpires)
            timeout = (utc2local(expires) - datetime.datetime.now()).seconds
            mem_cli.set_token('token', token.get('id'), timeout)
            token = token.get('id')
    elif mem_cli.get_token('token'):
        token = mem_cli.get_token('token')
    return token
'''
...
class MailUtil(object):
    ...
    def __init__(self, from_addr, password, smtp_host, smtp_port=25):
        self.from_addr = from_addr
        self.password = password
        self.smtp_host = smtp_host
        self.smtp_port = smtp_port
        self.server = smtplib.SMTP(smtp_host, smtp_port, timeout=20)
        # self.server.set_debuglevel(1)

    def send_mail(self, mail, to_mail_list, Cc_mail_list, path=None):
        """
        :param mail: mail content
        :param to_mail_list: []
        :param Cc_mail_list: []
        ...
        msg = MIMEMultipart()
        msg.attach(MIMEText(mail.get('Content', ''), 'plain', 'utf-8'))
        msg["Subject"] = Header(mail.get('Subject', ''), 'utf-8').encode()
        msg["From"] = _format_addr('%s <%s>' % (mail.get('From', self.from_addr), self.from_addr))
        if not to_mail_list:
            ...
            mail_list = []
            mail_list.extend(to_mail_list)
            addr_l = ''
            for addr in to_mail_list:
                addr_l = addr_l + "<%s>;" % _format_addr('<%s>' % addr)
            msg["To"] = addr_l
            if Cc_mail_list:
                addr_l = ''
                for addr in Cc_mail_list:
                    addr_l = addr_l + "<%s>;" %_format_addr('<%s>' % addr)
                msg["Cc"] = addr_l
                mail_list.extend(Cc_mail_list)
        if path:
            with open(path, 'rb') as f:
                mime = MIMEApplication(f.read())
                mime.add_header('content-disposition', 'attachment',
                                filename=os.path.split(path)[1].encode('gb2312'))
                mime.add_header('Content-ID', '<0>')
                mime.add_header('X-Attachment-Id', '0')
                msg.attach(mime)
        try:
            self.server.login(self.from_addr, self.password)
            self.server.sendmail(self.from_addr,
                                 mail_list,
                                 msg.as_string())
        except Exception as e:
            LOG.error("SendMail failed,catch exc: %s", str(e))
        finally:
            self.server.close()
