...
from report.api import api_version_request as api_version
from report.api import versioned_method
from report import exception
from report import i18n
from report.i18n import _
from report.i18n import _LE
...
class ResourceExceptionHandler(object):
    ...
    def __exit__(self, ex_type, ex_value, ex_traceback):
        ...
        if isinstance(ex_value, exception.Forbidden):
            ...
            print (_LE('Exception handling resource: %s'), ex_value, exc_info)
            raise Fault(webob.exc.HTTPBadRequest())
        elif isinstance(ex_value, Fault):
            print (_LI("Fault thrown: %s"), ex_value)
            raise ex_value
        elif isinstance(ex_value, webob.exc.HTTPException):
            print (_LI("HTTP exception thrown: %s"), ex_value)
...
class Resource(wsgi.Application):
    ...
    def get_body(self, request):
        try:
            ...
            print ("Unrecognized Content-Type provided in request")
            return None, ''
    ...
    def __call__(self, request):
        ...
        print "Resource __call__, req:", request
        ...
        if body:
            msg = _("Action: '%(action)s', calling method: %(meth)s, body: "
                    "%(body)s") % {'action': action,
                                   ...
                                   'meth': str(meth)}
            print (strutils.mask_password(msg))
        else:
            print ("Calling method '%(meth)s'",
                   {'meth': str(meth)})
    ...
    def dispatch(self, method, request, action_args):
        ...
        print "Resource dispatch, method:%s, request:%s, action_args:%s" \
              % (method, str(request), str(action_args))
        try:
...
class Fault(webob.exc.HTTPException):
    ...
    def __call__(self, req):
        ...
        print ("Returning %(code)s to user: %(explanation)s",
...
class RateLimitFault(webob.exc.HTTPException):
    ...
    def __call__(self, request):
        ...
        logger.debug("Resource __call__, req:" + str(request))
