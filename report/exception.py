...
from report.i18n import _, _LE
from report import safe_utils
...
class ReportException(Exception):
    ...
    def __init__(self, message=None, **kwargs):
        ...
        super(ReportException, self).__init__(message)

    def format_message(self):
        # NOTE(mrodden): use the first argument to the python Exception object
        # which should be our full ReportException message, (see __init__)
        return self.args[0]


class EncryptionFailure(ReportException):
    msg_fmt = _("Failed to encrypt text: %(reason)s")


class DecryptionFailure(ReportException):
    msg_fmt = _("Failed to decrypt text: %(reason)s")


class RevokeCertFailure(ReportException):
    msg_fmt = _("Failed to revoke certificate for %(project_id)s")


class VirtualInterfaceCreateException(ReportException):
    msg_fmt = _("Virtual Interface creation failed")


class VirtualInterfaceMacAddressException(ReportException):
    msg_fmt = _("Creation of virtual interface with "
                "unique mac address failed")


class VirtualInterfacePlugException(ReportException):
    msg_fmt = _("Virtual interface plugin failed")


class GlanceConnectionFailed(ReportException):
    msg_fmt = _("Connection to glance host %(host)s:%(port)s failed: "
                "%(reason)s")


class CinderConnectionFailed(ReportException):
    msg_fmt = _("Connection to cinder host failed: %(reason)s")


class Forbidden(ReportException):
...
class ImageNotActive(ReportException):
...
class ImageNotAuthorized(ReportException):
    msg_fmt = _("Not authorized for image %(image_id)s.")


class Invalid(ReportException):
...
class VolumeNotCreated(ReportException):
...
class HypervisorUnavailable(ReportException):
...
class ComputeServiceInUse(ReportException):
...
class ConstraintNotMet(ReportException):
    msg_fmt = _("Constraint not met.")
    code = 412


class NotFound(ReportException):
...
class AgentBuildExists(ReportException):
...
class NetworkDhcpReleaseFailed(ReportException):
    msg_fmt = _("Failed to release IP %(address)s with MAC %(mac_address)s")


class NetworkInUse(ReportException):
    msg_fmt = _("Network %(network_id)s is still in use.")


class NetworkSetHostFailed(ReportException):
...
class DuplicateVlan(ReportException):
    msg_fmt = _("Detected existing vlan with id %(vlan)d")
    code = 409


class CidrConflict(ReportException):
...
class NoMoreNetworks(ReportException):
...
class NetworkMissingPhysicalNetwork(ReportException):
...
class FixedIpExists(ReportException):
...
class FixedIpAssociateFailed(ReportException):
    msg_fmt = _("Fixed IP associate failed for network: %(net)s.")


class FixedIpAlreadyInUse(ReportException):
    msg_fmt = _("Fixed IP address %(address)s is already in use on instance "
                "%(instance_uuid)s.")


class FixedIpAssociatedWithMultipleInstances(ReportException):
...
class NoMoreFixedIps(ReportException):
    ec2_code = 'UnsupportedOperation'
...
class FloatingIpExists(ReportException):
...
class FloatingIpMultipleFoundForAddress(ReportException):
...
class FloatingIpAssociated(ReportException):
    ec2_code = "UnsupportedOperation"
    msg_fmt = _("Floating ip %(address)s is associated.")


class FloatingIpNotAssociated(ReportException):
...
class FloatingIpAllocateFailed(ReportException):
    msg_fmt = _("Floating IP allocate failed.")


class FloatingIpAssociateFailed(ReportException):
    msg_fmt = _("Floating IP %(address)s association has failed.")


class CannotDisassociateAutoAssignedFloatingIP(ReportException):
    ec2_code = "UnsupportedOperation"
...
class ServiceBinaryExists(ReportException):
    msg_fmt = _("Service with host %(host)s binary %(binary)s exists.")


class ServiceTopicExists(ReportException):
...
class QuotaExists(ReportException):
...
class OverQuota(ReportException):
...
class NoUniqueMatch(ReportException):
...
class ConsolePoolExists(ReportException):
...
class ConsolePortRangeExhausted(ReportException):
...
class FlavorExtraSpecUpdateCreateFailed(ReportException):
...
class CellExists(ReportException):
    msg_fmt = _("Cell with name %(name)s already exists.")


class CellRoutingInconsistency(ReportException):
...
class CellMaxHopCountReached(ReportException):
    msg_fmt = _("Cell message has reached maximum hop count: %(hop_count)s")


class NoCellsAvailable(ReportException):
    msg_fmt = _("No cells available matching scheduling criteria.")


class CellsUpdateUnsupported(ReportException):
...
class MalformedRequestBody(ReportException):
...
class AggregateError(ReportException):
...
class AggregateNameExists(ReportException):
...
class AggregateHostExists(ReportException):
    msg_fmt = _("Aggregate %(aggregate_id)s already has host %(host)s.")


class FlavorCreateFailed(ReportException):
    msg_fmt = _("Unable to create flavor")


class InstancePasswordSetFailed(ReportException):
...
class CouldNotFetchImage(ReportException):
    msg_fmt = _("Could not fetch image %(image_id)s")


class CouldNotUploadImage(ReportException):
    msg_fmt = _("Could not upload image %(image_id)s")


class TaskAlreadyRunning(ReportException):
    msg_fmt = _("Task %(task_name)s is already running on host %(host)s")


class TaskNotRunning(ReportException):
...
class InstanceUserDataTooLarge(ReportException):
    msg_fmt = _("User data too large. User data must be no larger than "
                "%(maxsize)s bytes once base64 encoded. Your data is "
                "%(length)d bytes")


class InstanceUserDataMalformed(ReportException):
    msg_fmt = _("User data needs to be valid base 64.")


class UnexpectedTaskStateError(ReportException):
...
class ServiceGroupUnavailable(ReportException):
    msg_fmt = _("The service from servicegroup driver %(driver)s is "
                "temporarily unavailable.")


class DBNotAllowed(ReportException):
...
class UnsupportedObjectError(ReportException):
    msg_fmt = _('Unsupported object type %(objtype)s')


class OrphanedObjectError(ReportException):
    msg_fmt = _('Cannot call %(method)s on orphaned %(objtype)s object')


class IncompatibleObjectVersion(ReportException):
    msg_fmt = _('Version %(objver)s of %(objname)s is not supported. The '
                'maximum supported version is: %(supported)s')


class ObjectActionError(ReportException):
    msg_fmt = _('Object action %(action)s failed because: %(reason)s')


class ObjectFieldInvalid(ReportException):
    msg_fmt = _('Field %(field)s of %(objname)s is not an instance of Field')


class CoreAPIMissing(ReportException):
    msg_fmt = _("Core API extensions are missing: %(missing_apis)s")


class AgentError(ReportException):
...
class InstanceGroupIdExists(ReportException):
...
class PluginRetriesExceeded(ReportException):
    msg_fmt = _("Number of retries to plugin (%(num_retries)d) exceeded.")


class ImageDownloadModuleError(ReportException):
...
class PciDeviceRequestFailed(ReportException):
    msg_fmt = _(
        "PCI device request (%requests)s failed")


class PciDevicePoolEmpty(ReportException):
    msg_fmt = _(
...
class PciRequestAliasNotDefined(ReportException):
    msg_fmt = _("PCI alias %(alias)s is not defined")


class MissingParameter(ReportException):
...
class InternalError(ReportException):
    ec2_code = 'InternalError'
    msg_fmt = "%(err)s"


class PciDevicePrepareFailed(ReportException):
    msg_fmt = _("Failed to prepare PCI device %(id)s for instance "
                "%(instance_uuid)s: %(reason)s")


class PciDeviceDetachFailed(ReportException):
    msg_fmt = _("Failed to detach PCI device %(dev)s: %(reason)s")


class PciDeviceUnsupportedHypervisor(ReportException):
    msg_fmt = _("%(type)s hypervisor does not support PCI devices")


class KeyManagerError(ReportException):
...
class RequestedVRamTooHigh(ReportException):
...
class NoLiveMigrationForConfigDriveInLibVirt(ReportException):
...
class UnshelveException(ReportException):
...
class SocketPortRangeExhaustedException(ReportException):
    msg_fmt = _("Not able to acquire a free port for %(host)s")


class SocketPortInUseException(ReportException):
...
class DatabaseMigrationError(ReportException):
